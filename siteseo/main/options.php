...
if('1' === siteseo_get_toggle_option('advanced')){
	...
	function siteseo_advanced_get_option($index){
		$options = get_option('siteseo_advanced_option_name');
		if(!empty($options) && isset($options[$index])) {
	...
	if ('1' === siteseo_get_service('AdvancedOption')->getBreadcrumbsJsonEnable()) {
		...
		add_action('init', 'siteseo_compatibility_wc');
		function siteseo_compatibility_wc() {
	...
	if ('1' == siteseo_advanced_get_option('advanced_comments_author_url')) {
		add_filter('get_comment_author_url', '__return_empty_string');
	}

	//Remove website field in comments	
	if('1' == siteseo_advanced_get_option('advanced_comments_website')){
	...
	function siteseo_load_advanced_admin_options(){
		...
		if('' != siteseo_advanced_get_option('appearance_adminbar')){
	...
	if('1' == siteseo_advanced_get_option('advanced_comments_form_link')){
	...
	//No /category/ in URL
	if('' != siteseo_advanced_get_option('advanced_category_url')){
	...
	if('' != siteseo_advanced_get_option('advanced_product_cat_url')){
