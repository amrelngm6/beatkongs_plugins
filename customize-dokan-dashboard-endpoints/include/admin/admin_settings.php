...
class cddep_add_settings_page_class {
	...
	public function add_admin_menus() {
        ...
        $general_cddepsettings_page = add_submenu_page( 'sysbasics', cddep_PLUGIN_name , cddep_PLUGIN_name , 'manage_woocommerce', esc_attr($this->cddep_notices_settings_page), array($this, 'plugin_options_page'));
	...
	public function cddep_options_tab_wrap() {
		...
		$current_tab = isset($_GET['tab']) ? $_GET['tab'] : "cddep_advanced_settings";
		...
		foreach ( $this->cddep_plugin_settings_tab as $tab_key => $tab_caption ) {
			...
			echo '<a class="nav-tab ' . esc_attr($active) . '" href="?page=' . esc_attr($this->cddep_notices_settings_page) . '&tab=' . esc_attr($tab_key) . '">' . esc_attr($tab_caption) . '</a>';	
	...
	public function get_accordion_content($key,$name,$core_fields,$value = null,$old_value = null,$third_party = null) {
        ...
        ?>
        <li keyvalue="<?php echo esc_attr($key); ?>" litype="<?php if (isset($value['cddep_type'])) { echo  $value['cddep_type']; } ?>" class="<?php if (isset($value['show']) && ($value['show'] == "no"))  { echo "cddep_disabled"; } ?> cddep_endpoint <?php echo esc_attr($key); ?> <?php if (isset($value['cddep_type']) && ($value['cddep_type'] == "group")) { echo 'group'; } ?> <?php if (preg_match('/\b'.esc_attr($key).'\b/', $core_fields )) { echo "core"; } ?>">
     ...
     public function get_main_li_content($key,$name,$core_fields,$value = null,$old_value = null,$third_party = null) { 
    	...
    	<h3>
    		<div class="cddep_accordion_handler">
    			<?php if (preg_match('/\b'.esc_attr($key).'\b/', $core_fields )) { ?>
    				<input type="checkbox" class="cddep_accordion_onoff" parentkey="<?php echo esc_attr($key); ?>"  <?php if (isset($value['show']) && ($value['show'] != "no"))  { echo "checked"; } elseif (!isset($value['show'])) { echo 'checked';} ?>>
    				<input type="hidden" class="<?php echo esc_attr($key); ?>_hidden_checkbox" value='<?php if (isset($value['show']) && ($value['show'] == "no")) { echo "no"; } else { echo 'yes';} ?>' name='<?php  echo esc_attr($this->cddep_notices_settings_page); ?>[<?php echo esc_attr($key); ?>][show]'>
    			...
    			<?php } else { 
    				...
    				?>
    				<span type="removeicon" parentkey="<?php echo esc_attr($key); ?>" class="dashicons dashicons-trash cddep_accordion_remove"></span>
    			<?php } ?>
    		</div>
        ...
        <div class="<?php echo $cddep_type; ?>_accordion_content">
        	...
        	<table class="cddep_table widefat">
        		...
        		<?php if (isset($third_party)) { ?>
        			...
        			<tr>
        				...
        				<td>
        					...
        					<input type="hidden" name="<?php  echo esc_attr($this->cddep_notices_settings_page); ?>[<?php echo esc_attr($key); ?>][third_party]" value="yes">
        					<input type="hidden" name="<?php  echo esc_attr($this->cddep_notices_settings_page); ?>[<?php echo esc_attr($key); ?>][endpoint_name]" value="<?php if (isset($name)) { echo esc_attr($name); } ?>">
        				</td>
                ...
                <?php if ((!preg_match('/\b'.esc_attr($key).'\b/', $core_fields ) && ($cddep_type == 'endpoint')) && (!isset($third_party))) { ?>   
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" class="cddep_accordion_input" name="<?php  echo esc_attr($this->cddep_notices_settings_page); ?>[<?php echo esc_attr($key); ?>][endpoint_key]" value="<?php if (isset($value['endpoint_key'])) { echo $value['endpoint_key']; } else { echo esc_attr($key); } ?>">
                    </td>
            
                </tr>
                <?php } else { ?>

            	    <input type="hidden" class="cddep_accordion_input" name="<?php  echo esc_attr($this->cddep_notices_settings_page); ?>[<?php echo esc_attr($key); ?>][endpoint_key]" value="<?php if (isset($value['endpoint_key'])) { echo $value['endpoint_key']; } else { echo esc_attr($key); } ?>">


                <?php  } ?>

        
                <input type="hidden" name="<?php  echo esc_attr($this->cddep_notices_settings_page); ?>[<?php echo esc_attr($key); ?>][cddep_type]" value="<?php echo esc_attr($cddep_type); ?>">

                <input type="hidden" name="<?php  echo esc_attr($this->cddep_notices_settings_page); ?>[<?php echo esc_attr($key); ?>][parent]" class="cddep_parent_field" value="<?php echo esc_attr($cddep_parent); ?>">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <input type="text" class="cddep_accordion_input" name="<?php  echo esc_attr($this->cddep_notices_settings_page); ?>[<?php echo esc_attr($key); ?>][endpoint_name]" value="<?php if (isset($value['endpoint_name'])) { echo $value['endpoint_name']; } else { echo ucfirst($key); } ?>">
                    </td>
                    ...
                    <td>
                    	...
                    	<div class="cddep_icon_settings_div">
                    		<div class="form-check cddep_icon_checkbox">
                    			<input class="form-check-input cddep_icon_source_radio" type="radio" name="<?php  echo esc_attr($this->cddep_notices_settings_page); ?>[<?php echo esc_attr($key); ?>][icon_source]"  value="default" <?php if ($icon_source == "default") { echo 'checked'; } ?>>
                    		...
                    		<div class="form-check cddep_icon_checkbox">
                    			<input class="form-check-input cddep_icon_source_radio" type="radio" name="<?php  echo esc_attr($this->cddep_notices_settings_page); ?>[<?php echo esc_attr($key); ?>][icon_source]"  value="noicon" <?php if ($icon_source == "noicon") { echo 'checked'; } ?>>
                    			<label class="form-check-label cddep_icon_checkbox_label">
                    				<?php  echo esc_html__('No icon','customize-dokan-dashboard-endpoints'); ?>
                    			</label>
                    		</div>
                    		<div class="form-check cddep_icon_checkbox">
                    			<input class="form-check-input cddep_icon_source_radio" type="radio" name="<?php  echo esc_attr($this->cddep_notices_settings_page); ?>[<?php echo esc_attr($key); ?>][icon_source]"  value="custom" <?php if ($icon_source == "custom") { echo 'checked'; } ?>>
                    			<label class="form-check-label cddep_icon_checkbox_label">
                ...
                <tr style= "<?php if ($icon_source == "custom") { echo 'display:table-row;'; } else { echo 'display:none;'; } ?>">
                    ...
                    <td>
                        ...
                        <input type="text" class="cddep_iconpicker icon-class-input" name="<?php  echo esc_attr($this->cddep_notices_settings_page); ?>[<?php echo esc_attr($key); ?>][icon]" value="<?php if (isset($value['icon'])) { echo $value['icon']; } ?>">
                ...
                <tr>
                    ...
                    <td>
                         <input class="cddep_accordion_input" type="text" name="cddep_advanced_settings[<?php echo esc_attr($key); ?>][link_inputtarget]" value="<?php if (isset($value['link_inputtarget']) && ($value['link_inputtarget'] != '')) { echo ($value['link_inputtarget']); } else { echo '#';} ?>" size="70">
                    </td>
                ...
                <tr>
			        ...
			        <td>
			            <select class="cddepvisibleto" name="cddep_advanced_settings[<?php echo esc_attr($key); ?>][visibleto]">
			    ...
			    <tr style="<?php if ((isset($value['visibleto'])) && ($value['visibleto'] == "specific")) { echo "display:table-row;"; } else { echo "display:none;"; } ?>" class="cddeproles">
			        ...
			        <td>
			            <select data-placeholder="<?php echo esc_html__('Choose Roles','customize-dokan-dashboard-endpoints'); ?>" name="cddep_advanced_settings[<?php echo esc_attr($key); ?>][roles][]" class="cddep_roleselect" multiple>
			    ...
			    <?php if (($cddep_type == 'endpoint') && (!preg_match('/\b'.esc_attr($key).'\b/', $exclude_content_core_fields )) && (!isset($third_party))) { ?>
			    ...
			    <tr>
                    ...
                    <td>    
                        ...
                        <?php 
                            ...
                            $editor_id      = 'cddep_content_'.esc_attr($key).'';
                            $editor_name    = ''.esc_attr($this->cddep_notices_settings_page).'['.esc_attr($key).'][content]';
                ...
                <?php if (($cddep_type == 'endpoint') && (preg_match('/\b'.esc_attr($key).'\b/', $extra_content_core_fields ))) { ?>
                		...
                		<td>
                			...
                			<div class="cddep_content_settings_div">
                				<div class="form-check cddep_content_checkbox">
                					<input class="form-check-input cddep_content_source_radio" type="radio" name="<?php  echo esc_attr($this->cddep_notices_settings_page); ?>[<?php echo esc_attr($key); ?>][content_settings]"  value="after" <?php if ($content_settings == "after") { echo 'checked'; } ?>>
                					<label class="form-check-label cddep_icon_checkbox_label" >
                						<?php  echo esc_html__('After Existing Content','customize-dokan-dashboard-endpoints','customize-dokan-dashboard-endpoints'); ?>
                					</label>
                				</div>
                				<div class="form-check cddep_content_checkbox">
                					<input class="form-check-input cddep_content_source_radio" type="radio" name="<?php  echo esc_attr($this->cddep_notices_settings_page); ?>[<?php echo esc_attr($key); ?>][content_settings]"  value="before" <?php if ($content_settings == "before") { echo 'checked'; } ?>>
                					<label class="form-check-label cddep_icon_checkbox_label">
                ...
                <?php if ($cddep_type == 'group') { ?>
                	...
                	<tr>
                		...
                		<td>    
                			<input class="cddep_accordion_input cddep_accordion_checkbox form-check-input" type="checkbox" name="cddep_advanced_settings[<?php echo esc_attr($key); ?>][group_open_default]" <?php if (isset($value['group_open_default']) && ($value['group_open_default'] == "01")) { echo 'checked'; } ?> value="01">
                		</td>
                	</tr>
                ...
                <tr>
                    ...
                    <td>    
                        <input type="text" class="cddep_accordion_input cddep_class_input" name="<?php  echo esc_attr($this->cddep_notices_settings_page); ?>[<?php echo esc_attr($key); ?>][class]" value="<?php if (isset($value['class'])) { echo $value['class']; } ?>">
                    </td>
                </tr>
            ...
            <?php if (($cddep_type == 'group') && ((isset($value['parent']) && ($value['parent'] == "none")) || ($key == "settings"))) {
