...
class Module {
    ...
    public function __construct() {
        ...
        add_filter( 'dokan_get_product_block_configurations', [ $this, 'get_product_block_config' ] );
    }
    ...
    public function handle_step_submission() {
        ...
        if ( isset( $_REQUEST['save_step'] ) && ! empty( sanitize_text_field( wp_unslash( $_REQUEST['save_step'] ) ) ) ) {
    ...
    public function do_ajax_product_import() {
        ...
        if ( ! ( current_user_can( 'edit_products' ) || dokan_is_user_seller( get_current_user_id() ) || current_user_can( 'dokan_import_product' ) ) ) {
    ...
    public function get_product_block_config( $config ) {
        $config['can_import'] = $this->can_import_products();
        $config['can_export'] = $this->can_export_products();
    ...
    public function can_import_products() {
        return current_user_can( 'dokan_import_product' );
    }
    ...
    public function can_export_products() {
        if ( ! current_user_can( 'dokan_export_product' ) ) {
            return false;
        }

        if ( ! apply_filters( 'dokan_csv_export_enabled', true ) ) {
            return false;
        }
