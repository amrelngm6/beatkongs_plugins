...
class Sonaar_Music_Pro {
    ...
    public function load() {
        if ( class_exists( 'Sonaar_Music' ) ){
            ...
            if (get_site_option('SRMP3_ecommerce') == '1'){
                ...
                add_action( 'elementor/frontend/the_content', [ $this, 'elementor_remove_cf_data' ] ); //Remove_cf_data if not required
            }
        }
    }
    /**
	...
	*/
	...
	public function elementor_remove_cf_data( $content ) { 
		if ( 
...
function load_share_by_ajax_callback() {
    ...
    if(Sonaar_Music::get_option('share_socialmedia_enable', 'srmp3_settings_share') != 'false'){
        ...
        if (in_array('twitter', $share_social)) {
            $social_html .= '<a href="https://twitter.com/intent/tweet?url=' . urlencode($link) . '" target="_blank" class="fab fa-twitter"></a>';
        }
