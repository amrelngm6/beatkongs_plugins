    ...
    */
    public function widget ( $args, $instance ){
           ...
           // if (isset($terms) && $terms !=='' && $terms != false){
            if ($category){
                $returned_data = $this->getAlbumsFromTerms(0, $category, $posts_not_in, $category_not_in, $posts_per_pages, false); 
            ...
            if(function_exists( 'run_sonaar_music_pro' ) &&  get_site_option('SRMP3_ecommerce') == '1'){
                if( $albums == 'favorites' ){
                    ...
                    $albums = [];
                    $favoriteTracks = $this->loadFavoriteList_Cookies();
                    if($favoriteTracks){
            ...
            if($playlatestalbum && $category == false && !$isFavorite){
            ...
            if( empty($albums) || $import_file && $isFavorite !== true ) {
                ...
                if (!$feed && !$trackSet && !$isFavorite && !$lazy_load){
                    return;
                }
            }
            ...
            $progress_bar_style = ( isset( $this->shortcodeParams['progress_bar_style'] ) ) ? $this->shortcodeParams['progress_bar_style'] : false; 
            ...
            $playlist = $this->get_playlist(null, $albumParsed, $category, $posts_not_in, $category_not_in, $title, $feed_title, $feed, $feed_img, $el_widget_id, $artwork, $posts_per_pages, $all_category, $single_playlist, $this->getOptionValue('reverse_tracklist'), $audio_meta_field, $repeater_meta_field, 'widget', $track_desc_postcontent, $import_file, $rss_items, $rss_item_title, $isFavorite);
            if ( !$playlist ) return;
        ...
        $ironAudioClass .= ( $isFavorite )? ' srp_player_is_favorite': '' ;
        ...
        if ( !$lazy_load &&  strpos($ironAudioClass, 'srp_tts_player') && empty($playlist['tracks'])) {
        ...
        foreach( $playlist['tracks'] as $key1 => $track){
            ...
            */
            ...
            $format_playlist .= ($hasTracklistSoundwave) ? '<div class="srp_soundwave_wrapper">' . $fakeWave . '</div>' : '';
        ...
        $tracklistClass = '';
        if( isset( $this->shortcodeParams['tracklist_soundwave_style']) && $this->shortcodeParams['tracklist_soundwave_style'] == 'mediaElement'  ){
            $tracklistClass .= ' sr_waveform_mediaElement';
...
<?php endif ?>
  ...
  <?php
    ...
    */
    ...
    private function checkIfFavorite($track, $postid, $favoriteList){
        if(!$favoriteList) return false;
      
        foreach($favoriteList as $key => $value) {
            if($postid == $value['postId']){
                if($value['trackPos'] == $track["track_pos"]){
    ...
    private function force_pushOptionalCTA($audioSrc, $postobj = null, $isFavorite = false, $cta_download_settings = null, $cta_link_settings = null, $cta_share_settings = null, $cta_favorite_settings = null){
        ...
        if(isset($postobj)){
            if($cta_favorite_settings == "true" || (isset( $this->shortcodeParams['force_cta_favorite']) && $this->shortcodeParams['force_cta_favorite'] == 'true')){
                $optional_storelist_cta = array_merge( $optional_storelist_cta, $this->push_favorite_storelist_cta( $postobj->ID,  $isFavorite ) );
            }
        }
    ...
    private function push_woocart_in_storelist($post, $is_variable_product = null, $wc_add_to_cart = false, $wc_buynow_bt = false){
        ...
        if( $wc_buynow_bt || get_post_meta($post_id, 'beat_type', true) == 'free' ) {
            ...
            $beatMP3Id = get_post_meta($post_id,'beat_mp3',true)  ;
            $pageUrl = !empty($beatMP3Id) ? wp_get_attachment_url($beatMP3Id) : get_post_meta($post_id,'beat_mp3_url',true)  ;
    ...
    private function push_favorite_storelist_cta($postId, $isFavorite = false){
        if (  !function_exists( 'run_sonaar_music_pro' )){
            return [];
		}
   ...
   // $start_time = microtime(true);
        ...
        if(function_exists( 'run_sonaar_music_pro' ) &&  get_site_option('SRMP3_ecommerce') == '1'){
            $favoriteList = $this->loadFavoriteList_Cookies();
        }
        ...
        if(!$category){
            ...
            if( function_exists( 'run_sonaar_music_pro' ) && Sonaar_Music::get_option('sticky_show_related-post', 'srmp3_settings_sticky_player') == 'true' && !$all_category && $single_playlist){
                ...
                // retrieve albums IDs. No terms are set.
                $sr_postypes = Sonaar_Music_Admin::get_cpt($all = true);
        ...
        if( $feed == '1' ){
            ...
            */
            if (is_array($albums) && count($albums) == 0) {
                $playlist = $this->importFile($import_file, null, $combinedtracks = true, $rss_items, $rss_item_title, $isFavorite, $favoriteList);
            }
            foreach ( $albums as $a ) {
                $playlist = $this->importFile($import_file, $a, $combinedtracks = true, $rss_items, $rss_item_title, $isFavorite, $favoriteList);
            ...
            foreach ( $albums as $i => $a ) {
                ...
                if ($album_tracks != '' && $trackSource == 'post'){ 
                    ...
                    */
                        ...
                        switch ($fileOrStream) {
                            case 'mp3':
                                if ( isset( $beatItem["beat_mp3"] ) ) {
                                    ...
                                    // $track_title = ( get_the_title($mp3_id) !== '' && $track_title !== get_the_title($mp3_id))? get_the_title($mp3_id): $track_title;
                                    // $track_title = html_entity_decode($track_title, ENT_COMPAT, 'UTF-8');
                                    $track_title = $beatItem['post_title'];
                        ...
                        $thumb_id = null;
                        $trackFavorited = $this->checkIfFavorite($album_tracks[$i], $a->ID, $favoriteList);
        ...
        if(!$playlist){
            ...
            $playlist['tracks'] = array_values($uniqueTracks);
        ...
        if ($fileType == 'csv'){
            ...
            foreach ($csv_rows as $csv_row) {
                ...
                if( isset($a) && isset($cta_download_settings)){
                    $trackFavorited = $this->checkIfFavorite($track, $a->ID, $favoriteList);
