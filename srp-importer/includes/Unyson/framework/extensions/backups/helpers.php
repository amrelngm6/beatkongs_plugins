...
function fw_ext_backups_unzip_partial($zip, $destination_dir, $last_entry = '', $timeout = 0) {
	...
	if ($last_entry) {
		while ($last_entry != $zipInstance->statIndex($i)['name']) {
			$i++;
		}
		$i++;
	}
	...
	while (time() < $max_time) {
		...
		if ($i >= $zipInstance->numFiles) {
			$result['finished'] = true;
			return $result;
		}
		

		$name = $zipInstance->statIndex($i)['name'];
		...
		$size = $zipInstance->statIndex($i)['size'];

	
		
		$content = $zipInstance->getFromIndex($i);
		$content = str_split($content, 10240);
